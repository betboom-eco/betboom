...
contract Concede is Op, ReentrancyGuard {
    ...
    uint256 public constant gameType = 2;
    ...
    mapping(uint256 => mapping(uint256 => mapping(uint256 => uint256))) indexMaxTake;
    ...
    function _checkBet(
        ...
        require(concedeInfo[cupID][mID].isOpen, "concede not open");
        ...
        require(poolTake.add(cache.maxTake) <= total.add(cache.beforeTake).add(cache.value), "Concede out take");
    ...
    function checkClaim(
        ...
        require(betAddr == address(this), "not bet this Concede");
        if(!factory.getCancle(cupID, mID)) {
            (,,, uint256 amount) = getResult(cupID, mID, totalID);
